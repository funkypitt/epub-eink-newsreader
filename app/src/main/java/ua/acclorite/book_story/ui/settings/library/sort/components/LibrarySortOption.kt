...
// Only the two orders that make sense for the library are exposed.
...
fun LazyListScope.LibrarySortOption() {
    items(librarySortOptions, key = { it.name }) {
        val settings = LocalSettings.current

        LibrarySortOptionItem(
            item = it,
            isSelected = settings.librarySortOrder.value == it,
            isDescending = settings.librarySortOrderDescending.value
        ) {
            if (settings.librarySortOrder.lastValue == it) {
                settings.librarySortOrderDescending.update(!settings.librarySortOrderDescending.lastValue)
            } else {
                settings.librarySortOrderDescending.update(true)
                settings.librarySortOrder.update(it)
            }
        }
    }
}
        ...
        StyledText(
            text = stringResource(
                when (item) {
                    ...
                    LibrarySortOrder.LAST_MODIFIED -> R.string.sort_order_last_modified
